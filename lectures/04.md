...
oryginalnie dla języka *Scheme*. Łączy w sobie wiele pomysłów programowanie funkcyjnego z efektywnością C++.
...
```
#### 3. List
Lista `(podwójnie)` wiązana, umożliwia szybkie **dopisywanie/usuwanie** elementów w **dowolnym miejscu**. Zapewnia dwukierunkowy dostęp **lioniowy**.
...
#### 1. Set i multiset
Pojemnik asocjacyjny, tworzący **posortowany zbiór** elementów. `Multiset` umożliwia trzymanie wielu elementów o tym samym kluczu.
...
#### 2. Map i multimap
Pojemnik asocjacyjny, tworzący zbiór par **klucz - wartość**. `Multimap` umożliwia trzymanie wielu elementów o tym samym kluczu.
...
* metody dotyczące rozmiaru
  ...
  * `max_size()` - zwraca maksymalną ilość elementów jaką może pomieścić pojemnik metody
